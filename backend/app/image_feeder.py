...
class ImageFeeder:
    def __init__(self, face_repository: FaceRepository, face_service: FaceService, session: AsyncSession):
    ...
    async def process(self, progress_cb: Callable[[str], Awaitable[None]] | None = None):
        ...
        for fn in new_files:
            ...
            for model in models:
                try:
                    faces_data = self._face_service.represent_face(
                        img_path=IMG_ORIG_DIR / fn)
                    ...
                    for data in faces_data:
                        face = FaceRegion(
                            ...
                            left_eye=list(
                                map(int, data['facial_area']['left_eye'])),
                            right_eye=list(
                                map(int, data['facial_area']['right_eye'])),
                            model=model,
