...
from app.app_config import DETECTOR_BACKEND, IMG_ORIG_DIR, IMG_TEMP_DIR, METRIC_DEFAULT, MODEL_DEFAULT, MODEL_THRESHOLDS
...
class FaceService:
    ...
    def represent_face(self, img_path):
        try:
            return DeepFace.represent(img_path=img_path,
    ...
    async def find_list(self, limit: int, search: str) -> List[FaceItem]:
        return [
            self.map_face_region_to_item(item)
            for item in await self._face_repository.find_random(limit, search)
        ]

    def create_preview(self, face: FaceRegion) -> str:
        new_fn = hashlib.sha1(face.filename.encode()).hexdigest(
        ) + f"{face.x}_{face.y}_{face.w}_{face.h}.jpg"

        crop_and_save(image_path=str(IMG_ORIG_DIR / face.filename),
    ...
    async def analyze_image(self, file: UploadFile) -> List[AnalyzeBox]:
        ...
        for data in faces_data:
            ...
            similar_faces = [
                item
                for item in similar_faces if item.distance <= MODEL_THRESHOLDS[MODEL_DEFAULT]
            ]

            output.append(AnalyzeBox(
    ...
    async def find_similar_by_image(self, file: UploadFile, x: int, y: int, w: int, h: int, limit: int) -> List[FaceSimilarItem]:
        ...
        return await self.find_similar_by_vector(target_vector=vector, model=MODEL_DEFAULT, metric=METRIC_DEFAULT, limit=limit)
    ...
    async def find_similar_by_vector(self, target_vector: np.ndarray, model: str, metric: str, limit: int) -> List[FaceSimilarItem]:
